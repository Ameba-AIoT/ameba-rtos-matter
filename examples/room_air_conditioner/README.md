...
- **ZAP File:** `room-air-conditioner-app.zap`
- **Device Type ID:** `0x0072` (Matter Room Air Conditioner)
...
|-----------------|------------------------|-----------------|
| **1** | Room Air Conditioner | Main functional endpoint for HVAC features |
| **2** | Humidity Sensor | Optional endpoint for humidity sensor device |
| **3** | Temperature Sensor | Optional endpoint for temperature sensor device |
...
| **Relative Humidity Measurement** | Measures and reports the current relative humidity level in the environment | Server |

> **Note:**
> These clusters collectively allow control of power state, temperature, and fan behavior for a Matter Room Air Conditioner.
> You may exclude or modify them according to your device requirements.
...
|----------------|-------------|----------------------------|
| `matter_driver_set_measured_temp_cb()` | Temperature callback | Registers a callback to handle **Temperature Measurement** cluster |
| `matter_driver_set_measured_humidity_cb()` | Humidity callback | Registers a callback to handle **Temperature Measurement** cluster |
...
| `matter_driver_fan_init()` | Fan initialization | Configures Fan Control cluster and synchronizes Matter attributes with the device's hardware state |
| `matter_driver_humidity_sensor_init()` | Humidity sensor initialization | Configures Relative Humidity Measurement cluster and synchronizes Matter attributes with the device's hardware state |
| `matter_driver_temperature_sensor_init()` | Temperature sensor initialization | Configures Temperature Measurement cluster and synchronizes Matter attributes with the device's hardware state |
...
|----------------|-----------------|
| **ZAP File** | `room-air-conditioner-app.zap` |
| **Main Example File** | `example_matter_room_air_conditioner.cpp` |
| **Main Matter Driver File** | `matter_drivers.cpp` |
| **Main Device Driver File** | `room_aircon_driver.cpp` `temp_hum_sensor_driver.cpp` |
...
### Build Matter Libraries
    ...
    make room_air_conditioner_port
...
### Build Matter Libraries
    ...
    make -C asdk room_air_conditioner_port
