...
This example demonstrates an implementation of the **Matter Refrigerator** device type.

## 📘 ZAP Configuration

- **ZAP File:** `refrigerator-app.zap`
- **Device Type ID:** `0x0070` (Matter Refrigerator)
...
|-----------------|------------------------|-----------------|
| **1** | Refrigerator | Main functional endpoint for the refrigerator device |
...
| **Refrigerator and Temperature Controlled Cabinet Mode** | Defines available refrigerator operating modes | Server |
| **Refrigerator Alarm** | Reports alarm events such as door open or close | Server |
...
|----------------|-------------|----------------------------|
| `matter_driver_gpio_level_irq_handler()` | GPIO interrupt handler | Handles GPIO interrupt events (e.g., door open/close) and triggers corresponding updates to Matter attributes. |
| `matter_driver_refrigerator_init()` | Device initialization | Initializes the device hardware for **Refrigerator** device |
| `matter_driver_refrigerator_set_startup_value()` | Startup configuration | Sets initial values for the refrigerator clusters and synchronizes Matter attributes with the device’s hardware state |
| `matter_driver_set_mode_callback()` | Mode control callback | Registers a callback to handle **Refrigerator Mode** cluster |
| `matter_driver_set_door_callback()` | Door state callback | Registers a callback to handle **Refrigerator Alarm** cluster |
...
|----------------|-----------------|
| **ZAP File** | `refrigerator-app.zap` |
| **Main Example File** | `example_matter_refrigerator.cpp` |
| **Main Matter Driver File** | `matter_drivers.cpp` |
| **Main Device Driver File** | `refrigerator_driver.cpp` |
| **Cluster Implementation File** | `ameba_tcc_mode.cpp` |
