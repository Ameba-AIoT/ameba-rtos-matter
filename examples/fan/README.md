...
This example demonstrates an implementation of the **Matter Fan** device type.

## 📘 ZAP Configuration

- **ZAP File:** `fan-app.zap`
- **Device Type ID:** `0x002B` (Matter Fan)
...
|-----------------|------------------------|-----------------|
| **1** | Fan | Main functional endpoint for the fan device |
...
|----------------|-------------|----------------------------|
| `matter_driver_fan_init()` | Fan initialization | Initializes the device hardware for **Fan** device |
| `matter_driver_fan_set_startup_value()` | Startup configuration | Sets initial values for the fan clusters and synchronizes Matter attributes with the device’s hardware state |
...
|----------------|-----------------|
| **ZAP File** | `fan-app.zap` |
| **Main Example File** | `example_matter_fan.cpp` |
| **Main Matter Driver File** | `matter_drivers.cpp` |
| **Main Device Driver File** | `fan_driver.cpp` |
