...
#if defined(CONFIG_EXAMPLE_MATTER_CAMERA) && CONFIG_EXAMPLE_MATTER_CAMERA

static void example_matter_camera_task(void *pvParameters)
{
    matter_wifi_wait();
...
extern "C" void example_matter_camera(void)
{
    if (xTaskCreate(example_matter_camera_task, ((const char *)"example_matter_camera_task"), 2048, NULL, tskIDLE_PRIORITY + 1, NULL) != pdPASS) {
        ChipLogProgress(DeviceLayer, "%s xTaskCreate(example_matter_camera_task) failed", __FUNCTION__);
    }
}
#endif /* CONFIG_EXAMPLE_MATTER_CAMERA */
