...
CHIP_ERROR PowerTopologyInit(chip::EndpointId endpointId)
{
    ...
    gPTInstance =
        std::make_unique<PowerTopologyInstance>(endpointId, *gPTDelegate,
...
CHIP_ERROR EvseApplicationInit()
{
    ...
    err = DeviceEnergyManagementInit(EndpointId(ENERGY_EVSE_ENDPOINT));
    if (err != CHIP_NO_ERROR)
    {
    ...
    err = EnergyEvseInit(EndpointId(ENERGY_EVSE_ENDPOINT));
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(AppServer, "Init failed in EnergyEvseInit");
        DeviceEnergyManagementShutdown();
        goto exit;
    }

    err = EnergyMeterInit(EndpointId(ENERGY_EVSE_ENDPOINT));
    if (err != CHIP_NO_ERROR)
    {
    ...
    err = PowerTopologyInit(EndpointId(ENERGY_EVSE_ENDPOINT));
    if (err != CHIP_NO_ERROR)
    {
    ...
    err = EVSEManufacturerInit();
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(AppServer, "Init failed in EVSEManufacturerInit");
        DeviceEnergyManagementShutdown();
        EnergyEvseShutdown();
        EnergyMeterShutdown();
        goto exit;
