...
static rtw_result_t _scan_result_handler(unsigned int scanned_AP_num, void *user_data)
{
	...
	scan_buf = (char *)os_mem_alloc(RAM_TYPE_DATA_ON, scanned_AP_num * sizeof(rtw_scan_result_t));
	if (scan_buf == NULL) {
		return RTW_ERROR;
	}

	if (wifi_get_scan_records(&scanned_AP_num, scan_buf) < 0) {
		os_mem_free((u8 *)scan_buf);
		return RTW_ERROR;
	}

	for (i = 0; i < BC_MAX_WIFI_SCAN_AP_NUM; i++) {
		...
		memcpy(&(BC_scan_result->ap_info[i]), scanned_AP_info, sizeof(rtw_scan_result_t));
		BC_scan_result->ap_num = i;
	}

	osif_mem_free((u8 *)scan_buf, 0);
...
int BC_req_scan_hdl(BC_band_t band, struct BC_wifi_scan_result* BC_scan_result)
{
	...
	scan_param.scan_user_callback = _scan_result_handler;

	BC_printf("Scan %s AP\r\n", (band == BC_BAND_2G)? "2.4G":"5G");
	os_sem_create(&wifi_scan_sema, 0, 1);
	BC_scan_result->ap_num = 0;
	ret = wifi_scan_networks(&scan_param,(void*) BC_scan_result);
	if(ret != RTW_SUCCESS) {
