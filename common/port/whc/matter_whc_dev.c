...
extern struct whc_dev_network_info whc_network_info[2];
...
#if defined(CONFIG_EXAMPLE_MATTER_DOORLOCK) && (CONFIG_EXAMPLE_MATTER_DOORLOCK == 1)
extern void matter_driver_doorlock_lock_state_callback(uint32_t id);
static void whc_matter_dev_doorlock_update(u8 lock_state)
{
    matter_driver_doorlock_lock_state_callback((u32) lock_state);
}
#endif
...
#if defined(CONFIG_EXAMPLE_MATTER_DOORLOCK) && (CONFIG_EXAMPLE_MATTER_DOORLOCK == 1)
    if (event == WHC_MATTER_DOWNLINK_DOORLOCK) {
        // DoorLock LockStateEnum: (0) NotFullyLocked, (1) Locked, (2) Unlocked, (3) Unlatched
        u8 state = *(buf + 1);
        whc_matter_dev_doorlock_update(state);
    }
#endif
}
...
void whc_matter_dev_uplink_hdl(u32 clusterId, u32 attributeId, const void *val, u16 len)
{
    ...
    u16 total_len =
        sizeof(u32) +     // magic
        sizeof(u32) +     // clusterId
        sizeof(u32) +     // attributeId
        sizeof(u16) +     // value length
        len;

    buf = rtos_mem_malloc(total_len);
    if (!buf) {
        return;
    }
    ...
    u32 magic = WHC_MATTER_UPLINK;

    memcpy(ptr, &magic, sizeof(u32));
    ptr += sizeof(u32);

    memcpy(ptr, &clusterId, sizeof(u32));
    ptr += sizeof(u32);

    memcpy(ptr, &attributeId, sizeof(u32));
    ptr += sizeof(u32);

    memcpy(ptr, &len, sizeof(u16));
    ptr += sizeof(u16);

    memcpy(ptr, val, len);
    ptr += len;

    //RTK_LOGI(TAG_MATTER_WHC, "whc dev send!\n");
    whc_dev_api_send_to_host(buf, total_len);
...
static s32 whc_dev_scan_callback(u32 scanned_AP_num, void *data)
{
    (void)data;
...
static void whc_dev_cmd_scan(void)
{
...
#endif
                rtos_mem_free(buf);
            }else if (event == WHC_MATTER_DOWNLINK) {
                whc_matter_dev_downlink_hdl(ptr);
            }
