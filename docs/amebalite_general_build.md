...
Navigate to the `ameba-rtos-matter` directory:
    ...
    ameba.py soc < RTL8720E / RTL8726E / ... >
...
If user wishes not to use the Matter helper command, please run the following command:
...
If user wishes not to use the Matter helper command, please run the following command:
    ...
    cd <build_RTL8726E/build build_RTL8726E/build>
...
The generated image is found in the ameba-rtos sdk `ameba-rtos-matter/build_RTL87XXX/`, both `km4_boot_all.bin` and `kr4_km4_app.bin` will be flashed.

Within `ameba-rtos-matter` folder, flash the image to the Ameba port (e.g. `/dev/ttyUSB0`).
...
The generated image is found in the ameba-rtos sdk `ameba-rtos/amebalite_gcc_project`, both `km4_boot_all.bin` and `kr4_km4_app.bin` will be flashed.
