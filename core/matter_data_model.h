...
/** Endpoint Macro Definitions */
/** Last fixed endpointId is (FIXED_ENDPOINT_COUNT - 1) */
#define LAST_FIXED_ENDPOINT_ID (FIXED_ENDPOINT_COUNT - 1)
/** First dynamic endpointId is located after the last fixed endpointId */
#define FIRST_DYNAMIC_ENDPOINT_ID (LAST_FIXED_ENDPOINT_ID + 1)
/** In the matter_core.cpp, the last fixed endpoint (placeholder EP) is going to be disabled */
/** So, enabled fixed endpoint count is (FIXED_ENDPOINT_COUNT - 1) */
#define ENABLED_FIXED_ENDPOINT_COUNT (FIXED_ENDPOINT_COUNT - 1)
...
private:
    ...
    chip::EndpointId endpointCount = ENABLED_FIXED_ENDPOINT_COUNT;
    chip::EndpointId nextEndpointId = FIRST_DYNAMIC_ENDPOINT_ID;
