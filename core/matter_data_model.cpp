...
{
    // If the endpoint is not part of dynamic endpoints, directly read from flash
    if (endpoint_id <= LAST_FIXED_ENDPOINT_ID) {
        char key[64];
        size_t len;
        int32_t ret;
        sprintf(key, "g/a/%x/%x/%x", cluster_id, cluster_id, matter_attribute->attributeId); // g/a/endpoint_id/cluster_id/attribute_id
        ret = getPref_bin_new(key, key, buffer, matter_attribute->size, &len);

        switch (ret) {
        case MATTER_KVS_SUCCESS:
            return Status::Success;
            break;
        case MATTER_KVS_ERR_NOT_FIND:
            return Status::NotFound;
            break;
        default: // MATTER_KVS_ERROR
            return Status::Failure;
            break;
        }
    } else { // Else it's the dynamic endpoint
        ...
        if (attribute->getAttributeSize() > max_read_length) {
            ChipLogError(DeviceLayer, "[%s] Insufficient space to read Attribute 0x%08x from Cluster 0x%08x in Endpoint 0x%04x", __FUNCTION__,
                         matter_attribute->attributeId, cluster_id, endpoint_id);
...
{
    // If the endpoint is not part of dynamic endpoints, directly write into flash
    if (endpoint_id <= LAST_FIXED_ENDPOINT_ID) {
        char key[64];
        int32_t ret;
        sprintf(key, "g/a/%x/%x/%x", cluster_id, cluster_id, matter_attribute->attributeId); // g/a/endpoint_id/cluster_id/attribute_id
        ret = setPref_new(key, key, buffer, matter_attribute->size);
...
void Endpoint::enableEndpoint()
{
    ...
    ChipError status = emberAfSetDynamicEndpoint(endpointIndex, endpointId, endpointType, chip::Span<chip::DataVersion>(dataVersion, clusters.size()),
