...
run_docker_build_scripts() {
  ...
  # Start the container
  docker run -d --name ameba-container ameba-rtos-matter:ameba-rtos-v1.2 tail -f /dev/null

  # Execute the setup and build script inside the container
  if ! docker exec ameba-container bash -c "
    chmod u+x ${tools_dir}/ameba-rtos/v1.2/amebagreen2/amebagreen2_build.sh  &&
    ${tools_dir}/ameba-rtos/v1.2/amebagreen2/amebagreen2_build.sh /opt/ameba
  "; then
