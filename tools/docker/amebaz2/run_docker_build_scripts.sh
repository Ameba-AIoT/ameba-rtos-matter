...
run_docker_build_scripts() {
  ...
  # Start the container
  docker run -d --name ameba-container ameba-rtos-matter:amebaz2 tail -f /dev/null

  # Execute the setup and build script inside the container
  docker exec ameba-container bash -c "
    chmod u+x ${tools_dir}/amebaz2/amebaz2_build.sh &&
    ${tools_dir}/amebaz2/amebaz2_build.sh /opt/ameba
  "
